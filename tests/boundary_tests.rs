...
fn test_empty_range_handling() {
    ...
    // 添加反向区间（start > end）
    let result = set.test_add(TestRange::new(20..10, true));
    assert!(result.is_ok());
    assert!(set.is_empty());

    // 删除空区间（应该是 no-op）
    let result = set.test_remove(10..10);
    assert!(result.is_ok());

    // 删除反向区间（应该是 no-op）
    let result = set.test_remove(20..10);
    assert!(result.is_ok());
}
...
fn test_boundary_overlap() {
    ...
    impl<T: Default> Default for TestRangeWithKind<T> {
        fn default() -> Self {
            Self {
    ...
    impl<T: core::fmt::Debug + Clone + Ord + Copy + Default> RangeInfo for TestRangeWithKind<T> {
        type Kind = u32;
        type Type = T;
...
fn test_single_point_ranges() {
    ...
    set.test_add(TestRange::new(6..7, true)).unwrap(); // 应该合并

    assert_eq!(set.len(), 1);
    assert_eq!(set.as_slice()[0].range(), (5..7));

    // 删除单点
    set.test_remove(6..6).unwrap(); // 删除空区间，应该 no-op
    assert_eq!(set.len(), 1);

    set.test_remove(5..6).unwrap(); // 删除第一个点
    assert_eq!(set.len(), 1);
...
fn test_extreme_values() {
    ...
    assert!(set.test_contains_point(i32::MAX - 1));
