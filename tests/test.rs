...
fn add_merges_overlaps_and_adjacency() {
    ...
    assert_eq!(set.as_slice(), vec![r(10, 40)]);

    // 相邻也会合并（[10,20) + [20,25) => [10,25)）
    set.clear();
    set.add_range(r(10, 20));
    set.add_range(r(20, 25));
    assert_eq!(set.as_slice(), vec![r(10, 25)]);
}
...
fn add_out_of_order_is_normalized() {
    ...
    assert_eq!(set.as_slice(), vec![r(0, 40)]);
}
...
fn remove_trims_and_splits() {
    ...
    assert_eq!(set.as_slice(), vec![r(10, 20), r(30, 50)]);

    // 删除左侧覆盖
    set.remove_range(r(0, 12));
    assert_eq!(set.as_slice(), vec![r(12, 20), r(30, 50)]);

    // 删除跨多个区间
    set.remove_range(r(15, 45));
    assert_eq!(set.as_slice(), vec![r(12, 15), r(45, 50)]);
}
...
fn remove_noop_on_empty_or_non_overlapping() {
    ...
    assert_eq!(set.as_slice(), vec![r(10, 20)]);
}
...
fn metadata_preserved_and_removed() {
    ...
    // 验证合并：[10,25) 包含两个原始区间
    assert_eq!(set.as_slice(), vec![r(10, 25), r(30, 40)]);
    ...
    assert_eq!(set.as_slice(), vec![r(20, 25), r(30, 40)]);
    ...
    assert_eq!(set.as_slice(), vec![r(20, 25), r(30, 32), r(35, 40)]);
    // "third" 仍在两个分裂的片段中
...
fn originals_merge_when_metadata_equal_and_adjacent() {
    ...
    assert_eq!(set.as_slice(), vec![r(10, 50)]);
    assert_eq!(set.elements()[0].originals.len(), 2);
    assert_eq!(set.elements()[0].originals[0].range, r(10, 40));
    assert_eq!(set.elements()[0].originals[0].meta, "same");
